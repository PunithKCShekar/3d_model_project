...
import { Camera, Layers, Play, Pause, FolderHeart, Plus, Trash2, Check, RefreshCw, HelpCircle } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-screen bg-bg-darker text-text-primary overflow-hidden font-sans">
      
      {/* Toast Notification */}
      {notification && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 px-4 py-3 bg-bg-panel border border-brand/30 rounded-lg shadow-2xl animate-fade-in-up backdrop-blur-md">
          <div className="w-2 h-2 rounded-full bg-brand animate-ping" />
          <span className="font-mono text-xs uppercase tracking-wider">{notification.message}</span>
        </div>
      )}

      {/* Main 3D Stage Section (Left 60% on desktop) */}
      <div className="relative w-full lg:w-[60%] h-[55vh] lg:h-full border-b lg:border-b-0 lg:border-r border-border-dark flex flex-col justify-between p-4 z-20">
      ...
      {/* Control Rail Section (Right 40% on desktop) */}
      <div className="w-full lg:w-[40%] h-[45vh] lg:h-full z-20">
        <ControlPanel
          config={config}
